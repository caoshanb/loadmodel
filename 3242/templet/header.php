<div class="Navigation" ng-controller="header" ng-cloak>
		...
		<ul class="nav" >
			...
			<li  ng-show="getJusr.project!=0 && getJusr!=undefined"  ng-click="actives(4);getUrls($event,4)" >
				<a href="./index.php?loadmodel=project">项目</a>
			</li>
			<li  ng-show="getJusr.auth_user!=0 && getJusr!=undefined"  ng-click="actives(5);getUrls($event,5)">
		...
		<ul class="userNav">
			...
			<li ng-click="userInfo()"  class="userInfo">
				...
				<ul ng-show="hideUserInfo">
					<li>账号：{{userInfos.user_account | cut:true:12:'...' }}</li>
					<li>户名：{{userInfos.user_name | cut:true:4:'...'}}</li>
					<li class="modifyPwd"><a href="javascript:;" ng-click="modifyPwd($event)" >修改密码</a></li>
					<li class="liBor">&nbsp;</li>
					<li >
						<span ng-show="userInfos.agent!=null"> 代理人:{{userInfos.agent | cut:true:4:'...'}}<a href="javascript:;" class="cancel" ng-click="cancelAgent($event)" >取消</a></span>
		...
		<div class="agents" ng-show="agent" >
			...
			<div>
				<form  name="setAgentList" id="setAgentList">
					...
					<p class="timeSelect">
						<label>设置时间</label>
		                    <input type="text" autocomplete="off" id="J-xl" value="" name="agent_start_time" placeholder="请选择"/><i class="icon iconfont icon-riqi1 iconfontss"></i>
		                    <span> 至 </span>
		                    <input type="text" autocomplete="off" id="J-xl2" value=""  name="agent_end_time"  placeholder="请选择"/><i class="icon iconfont icon-riqi1 iconfontss"></i>
					</p>
		...
		<div class="agents modifyPassword" style="height:350px;width:400px" ng-show="setPassword" >
			...
			<div>
				<form  name="setPwdList" id="setPasswordList" novalidate  >
					...
					<p class="timeSelect">
						<label>确认密码：</label> 
						<input type="password" name="repeatPwd" required placeholder="请输入" ng-blur="repeatPwds=true" ng-model="pwdList.re_password" >
