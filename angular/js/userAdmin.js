webModel.controller('userAdmin', ["$scope","$timeout","$window",'$compile','util', function ($scope,$timeout,$window,$compile,util) {
    ...
    $scope.getUserList=function(){
       ...
       var timestamp = Date.parse(new Date())/1000;
        ...
        util.httpRequest(urlss,"post",sendDate,true).then(function(datas){
            if(datas.status==1){
                var page_index=getStorage("page_index");
                if(page_index){
                    $scope.total = datas.data.page_total*$scope.page.page_rows;
                    $scope.page.page_index=page_index;
                    $scope.changeList(page_index);
                        ...
                        util.httpRequest(urlss,"post",sendDate,true).then(function(datas){
                            if(datas.status==1){
                                $.alerts({
                                    msg:datas.message,
                                    callback: function () {
                                        setStorage("page_index",$scope.page.page_index);
    ...
    $scope.reClose=function(){
        $scope.newUsers=false;
        angular.element(".attributeGroup input[type=checkbox]").attr("checked",false);
        angular.element(".jurisdictions input[type=checkbox]").attr("checked",false);
   ...
   $scope.updateSelected = function(action,id,name){
    ...
    $scope.getGroupLists=function(newArray){
        if($scope.getGroupList==undefined){
        var sendDate={};
        var timestamp = Date.parse(new Date())/1000;
    ...
    $scope.disableUser=function(user_account,r){
        if(r==1){
            var msgInfo="是否确认禁用该用户?";
        }else{
           var msgInfo="是否确认启用该用户?";
        }
        $.confirms({
            msg:msgInfo,
            callback:function(){
    ...
    $scope.resetPassWord=function(name){
            ...
            callback:function(){
                ...
                sendDate.new_password=$scope.initialPassword;
