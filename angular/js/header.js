webModel.controller('header', ["$scope", "httpAjax","$window" ,"util", function ($scope, httpAjax ,$window ,util) {
	$scope.notShow=false;
    ...
    $scope.getJusrList=function(){
		...
		util.httpRequest(urlss,"POST",sendDate,true).then(function(datas){
			if(datas.status==1){
                ...
                setStorage("getJusr",$scope.getJusr);
            }else{
                $.alerts({
                  msg:datas.message,
                  callback: function () {
	...
	$scope.getUserInfos=function(){
		...
		util.httpRequest(urlss,"POST",sendDate,false).then(function(datas){
			if(datas.status==1){
               ...
               sessionStorage.setItem("userInfor",JSON.stringify(datas.data));
	...
	$scope.getUrls=function(event){
		...
		if(u.indexOf("&")==-1){
			...
			sessionStorage.setItem("urlr",rs);
		}
	}
    $scope.getUrls();
}])