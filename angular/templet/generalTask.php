			         	...
			         	<form  id="taskList" >
				         	...
				         	<div class="bommit">
						        ...
						        <tr ng-hide="generalTaskList==''|| blsit"  ng-repeat="general in generalTaskList track by $index" >
						             ...
						             <td title="{{general.task_name}}">{{general.task_name |cut:true:10:'...'}}</td>
						             <td>{{general.task_process}}</td>
						             <td>{{general.consume_days}}</td>
						             <td>{{general.time_start}}</td>
						             <td>{{general.time_end_dev}}</td>
						             <td>{{general.time_end_test}}</td>
						             <td>
						             	<span ng-show="general.time_release!='' && general.is_released==1">{{general.time_release}}</span>
						             	<span ng-show="general.can_release==0 && general.time_release=='' && general.time_end_schedule!=''&& general.time_end_dev!='' && general.is_released !=1">计划{{general.time_end_dev}}</span>
						             	<span ng-show="general.can_release==1 && general.time_release==''&& general.time_end_dev!='' && general.is_released !=1 " 
						             	 class="notStart" ng-click="releaseTask(general.id)" >发布</span>
						             	<!-- <span 
						             	 class="notStart" ng-click="releaseTask(general.id)" >发布</span> -->
						             </td>
						        
						             <td title="{{general.remark}}">{{general.remark | cut:true:8:'...'}}</td>
						             <td>
						             ...
						             <td>
						             	...
						             	<a ng-show="general.is_plan!=undefined && general.is_plan==1" href="./index.php?loadmodel=myMissionPlan&xid={{general.id}}&auth_type=1&read={{general.is_r}}">计划</a>
						             	<a  href="./index.php?loadmodel=taskDetails&xid={{general.id}}&auth_type=1&read={{general.is_r}}">详情</a>
						             	<a href="javascript:;" ng-show="getJus.alltask==2" ng-click="removeMyAk(general.id)" >删除</a>
						             </td> 
						        </tr>
						...
						<!-- <form  class="timeSocl" name="Informations" novalidate >
							<div class="mout newTask">
								...
								<div >
									...
									<span class="errorInfo" ng-show="Informations.priorityr.$error.required && cg">请选择优先类型</span> 
								</div>
								<div >
									<label>耗时(天)：</label>
						...
						<form  class="timeSocl" name="Informations"  >
							<div class="mout newTask">
								...
								<div >
									<label>审批时间：</label>
									<span class="timeSel">
								            <input type="text" autocomplete="off" id="J-x6" value=""   name="s"  ng-model="editForm.time_approve"  placeholder="请选择"/  ><i class="icon iconfont icon-riqi1 iconfontss"></i>
					         		</span>
					         		<!-- <span class="errorInfo" ng-show="Informations.s.$error.required && s">审批时间不能为空</span>  -->
								</div>
								<div>
								...
								<div>
									...
									<span class="radio_right">
										...
										<label  style="text-align:left;">
											<input type="radio" name="is_r"   ng-click="editForm.is_plan='0';" >不需要
										</label>
										<!-- <span  class="errorInfo"  ng-show="is_plans">未选择计划</span> -->
									</span>
								</div>
							...
							<div class="mout userList">
								<div ng-repeat="user in addUserLIsts"   >
									<h4>{{user.group_name}}</h4>
										<label datas="{{user.id}}"  ng-repeat="us in user.group_member" style="display: inline-block;">
											<input type="checkbox" ng-model="not" vlaue="{{us.uid}}" class="groupUser" 
											 name="{{us.user_name}}" ng-click="so($event,us.uid,us,user.id,user.group_name)" ng-checked="isSelecteds(us.uid,user.id,user.group_name)" >{{us.user_name}}
										</label>
								</div>
							</div>
							<div class="but">
