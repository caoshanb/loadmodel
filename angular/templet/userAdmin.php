						...
						<div class="modal_main">
							<form id="timeSocl" class="timeSocl" name="newUserLists" >
								<div class="mout modal_our">
									...
									<div>
										<label>所属小组：</label>
										<!-- <select  ng-if="modifyUser" name="is_group" required ng-blur="isGroup=true" 
										 ng-change="leaderChange(editUser.group_id)" ng-options=" z.gid as z.name for z in getGroupList"
										 ng-model="editUser.group_id"  >
										...
										<div class="attributeGroup">
											<span ng-repeat="group in getGroupList" ng-if="modifyUser">
												<label><input type="checkbox"  name="groupBox" 
												 ng-checked="isCheckedo(group.gid)" ng-click="so($event,group.gid)"
												 />{{group.name}}</label>

											</span>
											<span ng-repeat="group in getGroupList" ng-if="!modifyUser">
												<label><input type="checkbox"  name="groupBox" 
												   ng-click="so($event,group.gid)"
												 />{{group.name}}</label>

											</span>
										</div>
										<span class="error"><span class="errorInfo" ng-show="newUserLists.is_group.$error.required && isGroup">所属小组不能为空</span></span>
									</div>
									<div>
										<label>负责人：</label>
										<select ng-if="modifyUser" name="is_person"  ng-blur="isPerson=true" ng-options="value.tid as value.name for value in leaderList" required ng-model="editUser.leader" >
											<option value="">请选择负责人</option>
										</select>
										<select ng-if="!modifyUser" name="is_person"  ng-blur="isPerson=true" ng-options="value.tid as value.name for value in leaderList" required ng-model="newUserList.leader" >
											<option value="">请选择负责人</option>
										</select>
										<span class="error"><span class="errorInfo" ng-show="newUserLists.is_person.$error.required && isPerson">负责人不能为空</span></span>
									</div>
									<div class="but" >
